...
public class TablesNamesFinderTest {
  ...
  @Test
  public void testGetTableListFromDelete3() throws Exception {
    String sql = "DELETE FROM MY_TABLE1 where enable = :enable and name = :name";
    net.sf.jsqlparser.statement.Statement statement = pm.parse(new StringReader(sql));
  ...
  public void testUnion() throws JSQLParserException {
    String sql = "select count(0) from (SELECT users.id, users.account, users.name FROM (SELECT"
        + " ac.id id, ac.account account, emp.`name` name FROM pt_account ac INNER JOIN pt_edu_emp emp ON ac.id = emp.id "
        + "WHERE ac.org_id = 'af02c94968114f519bbb2c8625b649b5' AND ac.deleted=0 AND ac.`enable`=1"
        + " UNION ALL SELECT ac.id id, ac.account account, tea.`name` name FROM pt_account ac INNER JOIN pt_teacher tea ON ac.id = tea.id"
        + " WHERE ac.org_id = 'af02c94968114f519bbb2c8625b649b5' AND ac.deleted=0 AND ac.`enable`=1 ) users"
        + " WHERE 1=1 and users.id not in (select account_id from pt_user_role where role_id in ('3001','3002')) ORDER BY users.account ASC,users.name ASC) as total";
    net.sf.jsqlparser.statement.Statement statement = pm.parse(new StringReader(sql));
  ...
  public void testGetTableListIssue194() throws Exception {
    String sql = "SELECT a.*,b.* from T1 a,T2 b where a.id=b.id and exists(select 1 from T3 c where c.enable=:enable)";
    net.sf.jsqlparser.statement.Statement statement = pm.parse(new StringReader(sql));
  ...
  @Test
  public void testUpdateGetTableJoin() throws JSQLParserException {
    Update statement = (Update) CCJSqlParserUtil
        .parse("UPDATE component Left JOIN t2 ON component.id= t2.id SET col = 0  WHERE component.name=afs");
    TablesNamesFinder tablesNamesFinder = new TablesNamesFinder();
    List<String> tableList = tablesNamesFinder.getTableList(statement);
    assertEquals(2, tableList.size());
    assertTrue(tableList.contains("component"));
    assertTrue(tableList.contains("t2"));
  }
