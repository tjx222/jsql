...
public class Table extends ASTNodeAccessImpl implements FromItem, MultiPartName {
    ...
    public String getRealName() {
		return NameAdapter.replaceTableName(this.name);
	}
    
    public String getBaseName() {
        return NameAdapter.replaceAlias(this.name);
    }
    ...
    public String getFullyQualifiedName() {
        ...
        if (database != null) {
            fqn.append(database.getFullyQualifiedName());
        }
        if (fqn.length() > 0) {
            fqn.append(".");
        }
